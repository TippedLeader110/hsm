...
<script type="text/javascript">
    ...
    for (var i = 0; i < FKaryawan.length; i++) {
        ...
        var st = '<tr><td class="nomorSorting">' + FPCount + '</td><td>A' + pgw.id + '</td><td>' + pgw.nama + '</td>';
    ...
    var orderRanking = () => {
        for (let index = 0; index < nilaiId.length; index++) {
            var col = table.columns(index + 3);
            col.visible(!col.visible());
        }

        var lastPost = table.rows(0).data()[0].length - 1;

        table
            .order([lastPost, 'desc'])
            .draw();

        $('td.nomorSorting').each(function(index, data) {
